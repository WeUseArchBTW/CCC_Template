 ...
 */
fun List<String>.firstTimesWindowedBy(parseWindowSize: (String) -> Int = { it.toInt() }, block: (List<String>) -> Any)
	= skip(1).timesWindowedBy(times = first().toInt(), parseWindowSize = parseWindowSize, block = block)

fun List<String>.timesWindowedBy(times: Int, parseWindowSize: (String) -> Int, block: (List<String>) -> Any): String {
	var index = 0
